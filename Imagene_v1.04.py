...
def BuildModel(train , Y_train , test , Y_test , method, params, cv_par, scoring_par, gridsearch, param_grid, select_label_var_list, data_type, label_type, trainmodel):
    ...
    if method in ['DecisionTree','LinearRegression', 'LinearModel' , 'LASSO']:
        if gridsearch == 'True':
        ...
        print("options are :DecisionTree, LinearRegression, LinearModel, LASSO", "SVM")
    if trainmodel:
        #try:
            '''
            performing training
...
if __name__ == "__main__":
    ...
    if(mode=="predict" or mode=="validate"):
        ...
        model_name_extract=load_model.split('.')[0]
